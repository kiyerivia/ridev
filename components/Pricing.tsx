...
export default function Pricing() {
  ...
  return (
    <section id="harga" className="py-24 relative overflow-hidden bg-gradient-to-b from-[#07070a] via-[#0d071a]/90 to-[#07070a]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#130b24] border border-pink-500/40 text-pink-400 text-xs font-bold uppercase tracking-wider mb-4 shadow-[0_0_20px_rgba(255,0,127,0.25)]">
            ...
            return (
              <div
                ...
                className={`rounded-3xl p-6 sm:p-7 flex flex-col justify-between relative transition-all duration-300 shadow-2xl ${
                  isPop
                    ? "bg-gradient-to-b from-[#1c0b30] via-[#140824] to-[#0d0517] border-2 border-pink-500 shadow-[0_0_35px_rgba(255,0,127,0.35)] scale-100 lg:-translate-y-2 holo-corners"
                    : "bg-[#0f091a]/95 backdrop-blur-xl border border-slate-800 hover:border-pink-500/40"
                }`}
              >
                {/* Popular / Best Seller Top Badge */}
                {plan.badge && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-pink-500 via-rose-500 to-red-500 text-white text-[10px] font-black uppercase tracking-wider px-3.5 py-1 rounded-full shadow-[0_0_15px_rgba(255,0,127,0.5)] flex items-center gap-1.5 whitespace-nowrap">
                    {plan.badge}
                  </div>
                )}
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300 mb-5 min-h-[34px] leading-relaxed">
                    {plan.targetAudience}
                  </p>

                  {/* Price Box */}
                  <div className="p-4 rounded-2xl bg-black/40 border border-slate-800/80 mb-5">
                    {plan.originalPrice && (
                    ...
                    <div className="mt-2.5 pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-[11px]">
                      ...
                      <span className="text-emerald-400 font-bold">{plan.renewalFee}</span>
                    </div>
                  </div>
                  ...
                  <div className="mb-6 pt-2 border-t border-slate-800/70">
                    <button
                      onClick={() => toggleExpand(plan.id)}
                      className="w-full flex items-center justify-between py-2 text-xs font-bold text-pink-400 hover:text-pink-300 transition-colors"
                      type="button"
                    >
                    ...
                    {isExpanded && (
                      <div className="mt-3 p-3.5 rounded-xl bg-black/50 border border-pink-500/20 text-xs space-y-3 animate-fadeIn">
                        <div>
                          <span className="text-[10px] text-slate-400 font-bold uppercase block">Halaman & Menu</span>
                          <span className="text-white font-medium text-[11px]">{plan.detailedSpecs.pages}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 font-bold uppercase block">Domain & Hosting</span>
                          <span className="text-slate-200 font-medium text-[11px] block">{plan.detailedSpecs.domain}</span>
                          <span className="text-slate-300 text-[10px] block mt-0.5">{plan.detailedSpecs.hosting}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 font-bold uppercase block">Email Bisnis</span>
                          <span className="text-slate-200 font-medium text-[11px]">{plan.detailedSpecs.email}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Fitur Tambahan</span>
                        ...
                        <div className="pt-2 border-t border-slate-800 flex items-center justify-between text-[10px]">
                          ...
                          <span className="text-amber-400 font-bold">{plan.detailedSpecs.turnaround}</span>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <a
                  ...
                  className={`w-full py-3.5 px-4 rounded-xl font-heading font-extrabold text-xs sm:text-sm text-center transition-all duration-300 flex items-center justify-center gap-2 group active:scale-95 ${
                    isPop
                      ? "bg-gradient-to-r from-[#ff007f] via-[#ff0055] to-[#ff0038] text-white hover:brightness-110 shadow-[0_0_20px_rgba(255,0,127,0.5)]"
                      : "bg-[#160d26] text-white hover:bg-pink-600 hover:text-white border border-pink-500/40"
                  }`}
                >
        ...
        <div className="mt-14 p-6 rounded-2xl bg-[#0e0819] border border-pink-500/30 text-center max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <a
            href="#kalkulator"
            className="shrink-0 text-xs font-bold text-pink-400 hover:text-pink-300 underline underline-offset-4"
          >
